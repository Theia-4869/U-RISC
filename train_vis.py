...
class Trainer:
    def __init__(self, args):
        ...
        self.experiment = wandb.init(project='U-RISC', resume='allow', anonymous='must', entity='theia')
        self.experiment.config.update(dict(epochs=args.epochs, batch_size=args.batch_size, learning_rate=args.lr, amp=self.amp))
    ...
    def training(self, epoch):
        ...
        for i, (image, target) in enumerate(tbar):
            ...
            with autocast(enabled=self.amp):
                ...
                focalloss = 0.5 * focal_loss(predicted_logits, target, alpha=self.args.alpha, power=2)
                fscoreloss = 0.5 * fscore_loss(predicted, target)
                nearedgeloss = 0.4 * near_edge_loss(predicted, target, kernel_size=self.args.kernel_size)
                loss =  focalloss + fscoreloss + nearedgeloss
            ...
            self.experiment.log({
                    ...
                    'focal loss': focalloss.item(),
                    'fscore loss': fscoreloss.item(),
                    'near edge loss': nearedgeloss.item(),
    ...
    def validation(self, epoch):
        ...
        if args.eval_train:
            ...
            self.eval(trainloader, epoch)

        # evaluate on validation set
        mean_f_score = self.eval(self.valloader, epoch)
    ...
    def eval(self, dataloader, epoch):
        ...
        for t, (image, target) in enumerate(tbar):
            if self.dataset == "simple":
                ...
                predicted = torch.zeros((tar_shape[0], tar_shape[1], 10240, 10240))
                ...
                for i in range(10):
                    for j in range(10):
                        ...
                        with torch.no_grad():
                            ...
                            predicted[:, :, 1024*i:1024*(i+1), 1024*j:1024*(j+1)] = part_predicted.cpu()

                f_score, precision, recall = F_score(predicted[:, :, :tar_shape[2], :tar_shape[3]], target)
                total_f_score += f_score
                total_precison += precision
                total_recall += recall
                tbar.set_description("F-score: %.3f, Precision: %.3f, Recall: %.3f" %
        ...
        mean_precision = total_precison / len(dataloader)
        mean_recall = total_recall / len(dataloader)
        self.experiment.log({
                            'f-score': mean_f_score,
                            'precision': mean_precision,
                            'recall': mean_recall,
                            'images': wandb.Image(image[0].cpu()),
                            'masks': {
                                'true': wandb.Image(target[0].float().cpu()),
                                'pred': wandb.Image(predicted[0].float().cpu()),
                            },
                            'step': self.iterations,
                            'epoch': epoch,
                        })
